  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def generateWordLists(remainder : Occurrences,wordsList : List[List[Word]]) : List[Sentence] = {
      ...
      remainder match {
        ...
        case _ => ((for ((occur, words) <- generatePicks(remainder)) yield (generateWordLists(subtract(remainder, occur), (wordsList :+ words)))).toList.foldLeft[List[Sentence]](List())((acc:List[Sentence],list:List[Sentence]) => acc ++ list))
      }
    }
    sentence match {
      case List() => List(List())
